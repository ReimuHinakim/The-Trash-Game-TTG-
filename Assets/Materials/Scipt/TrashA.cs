...
public class TrashA : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag == "Can" && itemNum < 6)
        ...
        if (collision.gameObject.tag == "Paper" && itemNum < 6)
        {
            item = "Paper";
            Destroy(collision.gameObject, 0.1f);
            guideText.text = "Go trash can to throw away";
        ...
        if (collision.gameObject.tag == "Hamburger" && itemNum < 6)
        {
            item = "Hamburger";
            Destroy(collision.gameObject, 0.1f);
            guideText.text = "Go trash can to throw away";
